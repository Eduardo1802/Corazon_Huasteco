    ...
    <Link to={enlace} target='_BLANK'>
      <BootstrapTooltip title={<Typography variant='subtitle1'>{alt}</Typography>} arrow placement="bottom" firstcolor={firstcolor} secondcolor={secondcolor}>
        <Avatar firstcolor={firstcolor} secondcolor={secondcolor} issmallscreen={issmallscreen}>
          <img src={imagen} alt={alt} preload="true" cached="true" width={"150px"} height={"150px"} />
        </Avatar>
      </BootstrapTooltip>
    </Link>
  )
...
const BootstrapTooltip = styled(({ className, ...props }) => ( <Tooltip {...props} arrow classes={{ popper: className }} TransitionComponent={Zoom} /> ))(({ theme, firstcolor, secondcolor }) => ({
  [`& .${tooltipClasses.arrow}`]: { color: firstcolor ? firstcolor : theme.palette.primary.dark, },
  [`& .${tooltipClasses.tooltip}`]: { background: `linear-gradient(to bottom, ${ firstcolor ? firstcolor : theme.palette.primary.dark}, ${ secondcolor ? secondcolor : theme.palette.primary.light})`, },
}));
...
export const Avatar = styled(Box)(({ theme, firstcolor, secondcolor, issmallscreen }) => ({
  ...
  '&:hover': { cursor: "pointer", padding: "5px", transform: "rotate(10deg)",}
}))